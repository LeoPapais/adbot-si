...
public class UsuarioData {
    ...
    public VisualizacaoAdministradorDTO visualizarPerfilAdministrador(int Usuario_ID, Transacao tr) throws Exception {
        
        Connection con = tr.obterConexao();

        String sql = "select UserName, Nome, Sobrenome, E_mail, Senha, Tipo_Usuario, Data_de_cadastro";
        PreparedStatement ps = con.prepareStatement(sql);
        ps.setInt(1, Usuario_ID);
        ResultSet rs = ps.executeQuery();
        System.out.println("Query executada");
        ...
        VisualizacaoAdministradorDTO u = new VisualizacaoAdministradorDTO();
        u.setUserName(rs.getString("UserName"));
        u.setNome(rs.getString("Nome"));
        u.setSobrenome(rs.getString("Sobrenome"));
        u.setE_mail(rs.getString("E_mail"));
    ...
    public int editarPerfilAdministrador(int usuario_ID, UpdateAdministradorDTO updateUsuarioDTO, Transacao tr) throws Exception {
        System.out.println("UserData: profEdit: Begin.");
        int update_result = 0; // valor de atualizacoes no database
        Connection con = tr.obterConexao();
        
        // 1) Construcao das sentencas da query
        String query_block          = "select U.UserName, U.Nome, U.Sobrenome, U.E_mail, U.Senha, U.Tipo_Usuario from Usuario U where ID = ?";
        String update_UserName      = "update Usuario set UserName = ?       where ID = ?";
        String update_Nome          = "update Usuario set Nome = ?           where ID = ?";
        String update_Sobrenome     = "update Usuario set Sobrenome = ?      where ID = ?";
        String update_E_mail        = "update Usuario set E_mail = ?         where ID = ?";
